\documentclass[twoside, 10pt, twoside]{article}

\usepackage{amsmath}
\usepackage{graphicx} %[pdftex]
\usepackage[hidelinks]{hyperref}
\usepackage[inner=3cm,outer=2cm]{geometry}
\usepackage{fancyhdr}
\usepackage{pdfpages}
\usepackage[lined,ruled]{algorithm2e}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{pdflscape}

\title
{
	[H03I7A] Design in Medical Technology\\
	Shape-based feature analysis for\\
	nodule detection in lung images
}
\author{Kim Nuyts \and Sven Van Hove}
\date{\today}

\setlength{\headheight}{15.2pt}
\pagestyle{fancy}
\fancyhf{}
\fancyhf[HLE]{\leftmark}
\fancyhf[HRO]{\rightmark}
\fancyhf[FLE,FRO]{\thepage}

\numberwithin{equation}{section}

\hyphenation{iso-tropic}
\hyphenation{data-base}
\hyphenation{data-set}
\hyphenation{pleu-ral}
\hyphenation{no-du-les}
\hyphenation{an-noun-ced}
\hyphenation{Ca-na-da}

\begin{document}

\includepdf[pages=1]{coverpage.pdf}
\pagenumbering{arabic}
\onecolumn
%\maketitle
\tableofcontents
\clearpage

% paragraph makeup, after ToC
\setlength{\parindent}{0pt} %don't indent new paragraph
\setlength{\parskip}{2ex plus 1ex minus 1ex} %add blank line in between

\input{sections/summary.tex}
\clearpage
\twocolumn

\input{sections/introduction.tex}

\input{sections/literature.tex}

\input{sections/theory.tex}

\input{sections/methodology.tex}

\input{sections/results.tex}

\input{sections/conclusions.tex}

\clearpage
\onecolumn
\bibliographystyle{alpha}
\bibliography{references} \addcontentsline{toc}{section}{References}

\clearpage
\appendix
\begin{landscape}
\section{Appendix}
%\subsection{Mission definition}
%\includepdf[pages={-}]{appendix/mission.pdf}

\subsection{Gantt chart \& logbook}
\begin{figure}[htp]
  \includegraphics[width=\linewidth]{appendix/gantt.png}
  \caption{Gantt chart}
  \label{fig:gantt}
\end{figure}

\includepdf[pages={-}, landscape]{appendix/logbook.pdf}
\end{landscape}

\subsection{Meetings}
\includepdf[pages={-}]{appendix/1.pdf}
\includepdf[pages={-}]{appendix/2.pdf}
\includepdf[pages={-}]{appendix/3.pdf}
\includepdf[pages={-}]{appendix/4.pdf}
\includepdf[pages={-}]{appendix/5.pdf}
\includepdf[pages={-}]{appendix/6.pdf}
\includepdf[pages={-}]{appendix/7.pdf}
\end{document}