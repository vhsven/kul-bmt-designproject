...
types of nodules. However, these
...
while refusing as much false positives (\autoref{fig:Nonnodules}) as possible.
...
pulmonary (solid) nodules and micronodules \cite{median}. And Siemens has
developed the syngo.CT Lung CAD\texttrademark. They claim it is ``a fully
automated computer assisted second reader tool'' that is designed to assist
radiologists in the detection of solid pulmonary nodules \cite{siemens}.
...
chest low-dose CT scans. In the first step the lung tissues were isolated
by applying an iterative Markov-Gibbs random field (MGRF) based segmentation
...
algorithm. The third step eliminated the FP using three
...
statistical characteristics from a training set (20 FP, 20 lung TP, 20
lung wall TP nodules) were selected from 50 separate subjects. CT scans
from 200 subjects were tested in this study and a sensitivity of 82,3\% and a
FP rate of 9,2\% were reached. The algorithm, that was implemented in C++ on an Intel dual
processor with 16 GB memory, took about 5 minutes to process
182 CT slices of size 512 x 512.
...
distuingish solitary nodules from attached ones. When this algorithm was used to
...
image features -- shape index and curvedness -- to detect candidate nodules. Two
...
\subsubsection{Performance of existing systems} \label{sec:performance}
 The algorithms presented in a wide range of papers report varying successes in
 the automated detection of nodules. However, it is very difficult to compare
 studies against one another in a meaningful way due to differences in the size
 of the datasets, the evaluation methods, the data selection criteria and the
 characteristics of the nodules under examination \cite{lee2010}. Especially
 ...
 ginneken, mur}. Some studies focus on nodules below or above a certain size or
 on special types of nodules (e.g. solid nodules). \cite{mur} performed an
 extended literature review and found that the number of scans used for testing
 varied between 5 and 500 with a median number of 29,5. Many of the studies
 included multiple scans from individual patients, which means that the
